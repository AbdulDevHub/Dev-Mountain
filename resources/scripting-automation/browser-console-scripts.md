...
### 8. Turn the Page Into an Editable Document

Activates `designMode`, which lets you click anywhere on the page and type, delete, or edit text directly in the browser window. Run it once to turn it on, and run the cleanup script to turn it off.

```javascript
(function () {
  if (document.designMode === 'on') {
    document.designMode = 'off';
    console.log('Design mode off. Text editing disabled.');
  } else {
    document.designMode = 'on';
    console.log('Design mode on. Click anywhere on the page to edit text!');
  }
})();

---

### 9. Highlight Broken Images
...
### 10. Inspect Local Storage and Session Storage
