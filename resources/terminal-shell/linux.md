...
grep -n "pattern" file.txt      # display line numbers with matches
...
diff file1 file2                # show exactly what is different between two files
cmp file1 file2                 # compare two files (byte-level)
xargs                           # build/execute commands from stdin
bc                              # perform math calculations in the terminal
```
...
cat access.log | grep "404"      # filter log lines containing "404"
ls -la > directory_structure.txt # save directory listing to file
...
shutdown -h now             # power off the machine
```
...
curl checkip.amazonaws.com  # retrieve your public IP address
curl wttr.in/<location>     # display a text-based weather forecast
curl qrenco.de/<url>        # generate an ASCII QR code for a URL
curl -Is <url>              # check the HTTP response status header of a website
...
whatis ls                 # get a brief description of a command
```
...
shred -u -n 3 secret.pem         # securely overwrite and delete a file
```
...
man command                # manual page (press 'q' to exit)
